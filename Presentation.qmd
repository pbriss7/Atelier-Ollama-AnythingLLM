...
2.  S'ils sont utilisés localement, sur l'ordinateur personnel d'un utilisateur situé au Québec, l'énergie consommée provient d'une source qui **ne produit pas de GES**.
3.  En utilisant des modèles plus légers, localement, **aucune donnée n'est partagée** avec des tiers, pas même avec Ollama.
4.  Si on souhaite utiliser des modèles plus performants tout en conservant les avantages d'une utilisation privée, on peut recourir aux serveurs d'Ollama, qui garantissent la confidentialité des échanges.
...
-   Ollama ne crée pas de nouveaux modèles : il donne accès à ceux développés par la communauté ou libérés par les grandes compagnies (gemma2, llama3, qwen, etc.).
-   Ollama garantit la confidentialité des échanges avec les LLMs. Lorsqu'on utilise Ollama avec des modèles importés sur son ordinateur personnel, les données ne quittent pas l'ordinateur. Si on souhaite utiliser les serveurs d'Ollama et transmettre des appels à des modèles hébergés dans les centres de données d'Ollama, nos données restent confidentielles.
...
-   Compatible avec Ollama pour l'utilisation de modèles locaux
-   Peut servir de solution unique pour utiliser des modèles commerciaux
...
Si Ollama n'a pas déjà été installé sur l'ordinateur, c'est le moment de le faire. Il faut également créer un compte (gratuit) si on souhaite utiliser les modèles distants.
...
-   **Tâches intermédiaires** (analyse de texte, feedback pédagogique) : `gpt-oss:20b` (13 GB), `qwen3:8b`
-   **Tâches complexes** (évaluation détaillée, correction nuancée) : modèles cloud comme `gpt-oss:120b-cloud`, `deepseek-v3.1:671b-cloud`

# Deuxième partie : Paramétrage avancé

## Ajuster les paramètres avancés: la température
...
La température est un paramètre numérique qui contrôle le degré de **déterminisme** vs **créativité** dans les réponses du modèle. Elle s'exprime généralement sur une échelle de 0 à 2, bien que la plage usuelle se situe entre 0 et 1.

**Température basse (0 - 0.3)** : - Le modèle privilégie systématiquement les tokens ayant la probabilité la plus élevée - Les réponses sont plus **déterministes**, **cohérentes** et **factuelles** - Idéal pour corrections grammaticales, réponses factuelles, analyses structurées, tâches nécessitant précision et constance - Risques : réponses répétitives, manque de variation stylistique

**Température moyenne (0.4 - 0.7)** : - Équilibre entre prévisibilité et variabilité - Les réponses restent cohérentes tout en introduisant une certaine diversité - Idéal pour la plupart des tâches pédagogiques, feedback personnalisé, génération d'exercices - Configuration par défaut de nombreux modèles

**Température élevée (0.8 - 2.0)** : - Le modèle explore des tokens moins probables - Les réponses deviennent plus **créatives**, **variées** et **imprévisibles** - Idéal pour brainstorming, génération d'idées créatives, variation stylistique - Risques : incohérences, erreurs factuelles, divagations (*hallucinations*)
...
**Exercice pratique** : Testez le même prompt avec différentes températures

Prompt : *"Propose trois activités pédagogiques pour enseigner le passé composé à des apprenants de niveau A2."*

-   À température 0.2 : notez la constance des réponses
-   À température 0.7 : observez l'équilibre créativité/cohérence
-   À température 1.2 : constatez la diversité (et les possibles incohérences)

### Recommandations pour l'enseignement du FLS

| Tâche pédagogique | Température recommandée | Justification |
|------------------------|-----------------------------|-------------------|
| Correction grammaticale | 0.1 - 0.3 | Précision maximale requise |
| Feedback sur production écrite | 0.4 - 0.6 | Équilibre entre cohérence et personnalisation |
| Génération d'exercices | 0.5 - 0.7 | Variation nécessaire, cohérence maintenue |
| Brainstorming pédagogique | 0.8 - 1.0 | Créativité encouragée |
| Simplification de textes | 0.3 - 0.5 | Fidélité au contenu original |

### Autres paramètres disponibles

Bien que nous nous concentrions sur la température dans cet atelier, AnythingLLM permet d’ajuster d’autres paramètres qui influencent la génération :

-   Top P (nucleus sampling) : limite la sélection aux tokens dont la probabilité cumulée atteint un certain seuil
-   Top K : restreint la sélection aux K tokens les plus probables
-   Max tokens : définit la longueur maximale de la réponse
-   Repeat penalty : pénalise la répétition de tokens déjà générés
-   Pour une utilisation pédagogique standard, les valeurs par défaut de ces paramètres sont généralement appropriées.
...
En s’inspirant du Prompt Canvas, un message système bien conçu devrait inclure les éléments suivants :

#### 1. RÔLE ET EXPERTISE

Définissez clairement qui est le modèle et quelle expertise il incarne.
...
Précisez le type de personne à qui le modèle s'adresse: quel est son statut, sa catégorie d'âge, etc.
...
Établissez le registre de langue et le ton à adopter avec l'interlocuteur.
...
Définissez les limites, les principes éthiques et les priorités.
...
Spécifiez comment le modèle doit organiser ses réponses.
...
Suggère des activités ou ressources pour travailler les points faibles.
...
**Important** : le message système s’appliquera à tous les sujets (*threads*) de cet espace de travail. Pour des tâches différentes nécessitant des messages système différents, créez des espaces de travail distincts (ex: un espace “Évaluation”, un espace “Génération d’exercices”, etc.).
